...
class CollectionPermissionLookupMixin:
    ...
    def _check_perm(self, user, actions, collection=None):
        ...
        return self._users_with_perm(actions, collection=collection).filter(
            pk=user.pk
        ).exists()
...
class CollectionOwnershipPermissionPolicy(
    ...
    def user_has_any_permission_for_instance(self, user, actions, instance):
        ...
        if "change" in actions or "delete" in actions:
            # check if the user has permission to change the instance, or if they own it
            return (
                self._check_perm(user, real_actions, collection=instance.collection)
                or getattr(instance, self.owner_field_name) == user
            )
        else:
            # check if the user has permission to perform the action on the instance
    ...
    def collections_user_has_any_permission_for(self, user, actions):
        ...
        if user.is_active and user.is_superuser:
            ...
            # - for which they have 'change' permission
            # - OR for which they have 'add' permission, and
            #   contain at least one item owned by them

            change_perm_filter = Q(
                pk__in=list(self._collections_with_perm(user, ["change"]))
            )

            add_perm_filter = Q(
                pk__in=list(self._collections_with_perm(user, ["add"]))
            ) & Q(items__in=self.model.objects.filter(**{self.owner_field_name: user}))

            return Collection.objects.filter(change_perm_filter | add_perm_filter).distinct()
        else:
            # action is either not recognised, or is the 'add' action which is
...
class CollectionMangementPermissionPolicy(
    ...
    def _descendants_with_perm(self, user, action):
        ...
        return Collection.objects.filter(
            id__in=self._collections_with_perm(user, [action])
